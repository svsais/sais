# Saki's Arch Installer v2.1.3 - Archiso Portion
echo "---------------------------------------"
echo "Welcome to Saki's Arch Installer v2.1.3"
echo "---------------------------------------"
...
echo -n "# Autogenerated Variable Transfer" > /mnt/chr.sh
echo "" >> /mnt/chr.sh
vtrans () {
	echo -n $1 >> /mnt/chr.sh
}
...
vnext "sdrive_count=$sdrive_count"
vtrans "sdrive_ids=("
for d in ${sdrive_ids[@]}; do
	vtrans "\"$d\" "
done
...
vnext ")"
vnext "q_hostname=\"$q_hostname\""
vnext "q_root_password=\"$q_root_password\""
vnext "q_sdrive_symlink=\"$q_sdrive_symlink\""
vnext "q_ssh=\"$q_ssh\""
vnext "q_reflector=\"$q_reflector\""
vnext "q_zsh=\"$q_zsh\""
sdone
